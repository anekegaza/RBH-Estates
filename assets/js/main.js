...
const search = () => {
    ...
    const landAreas = document.getElementById("land-list")
    const lands = document.querySelectorAll(".lands")
    const landName = document.getElementsByTagName("h3")

    for(var i=0; i < landName.length; i++) {
        let match = lands[i].getElementsByTagName("h3")[0];

        if(match){
            let textvalue = match.textContent || match.innerHTML

            if(textvalue.toUpperCase().indexOf(searchBox) > -1){
                lands[i].style.display = "";
            }else{
                lands[i].style.display = "none";
            }
        }
    }
}
